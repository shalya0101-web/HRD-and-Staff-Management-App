...
export default function SlipGaji() {
  ...
  function hitungTotal(p) {
    ...
    if (tipe === 'gaji') {
      const pemasukan = (p.gaji_pokok||0) + (p.tunjangan_makan||0) + (p.tunjangan_transport||0) +
        ...
        (p.sip||0) + (p.lembur||0)
      const bpjsTotal = (p.potongan_bpjs_kes ?? 0) + (p.potongan_bpjs_naker ?? 0) || (p.potongan_bpjs||0)
  ...
  function generateSlipHTML(emp, p) {
    ...
    const komponenGaji = tipe === 'gaji' ? `
      ...
      ${rowGaji('BPJS Kesehatan', p.potongan_bpjs_kes, true)}
      ${rowGaji('BPJS Ketenagakerjaan', p.potongan_bpjs_naker, true)}
      ${(!p.potongan_bpjs_kes && !p.potongan_bpjs_naker) ? rowGaji('BPJS', p.potongan_bpjs, true) : ''}
      ${rowGaji('PPh 21', p.potongan_pph21, true)}
      ...
      <tr style="background:#f9fafb;border-top:1px solid #e5e7eb">
        ...
        <td style="padding:6px 8px;text-align:right;font-weight:600;font-size:13px;color:#dc2626">
          - Rp ${FMT(((p.potongan_bpjs_kes ?? 0)+(p.potongan_bpjs_naker ?? 0) || (p.potongan_bpjs||0))+(p.potongan_pph21||0)+(p.potongan_cicilan||0)+(p.potongan_kasbon||0)+(p.potongan_absensi||0)+(p.potongan_arisan||0)+(p.potongan_lainnya||0))}
        </td>
      </tr>
    ` : `
      <tr style="background:#f0f9ff">
...
<body>
  ...
  <div class="slip">
    ...
    <!-- Tanda Tangan (saat lunas) -->
    ${p.status === 'lunas' ? `
    <div style="display:flex;justify-content:flex-end;margin:10px 20px 0;padding:0 10px">
  ...
  <div class="no-print" style="text-align:center;margin-bottom:16px">
    ...
    empsWithPayroll.forEach((emp, idx) => {
      ...
      <div class="slip${idx < empsWithPayroll.length-1 ? ' page-break' : ''}">
        ...
        ${p.status === 'lunas' ? `
        <div style="display:flex;justify-content:flex-end;margin:0 20px 16px;padding:0 10px">
          <div style="text-align:center;min-width:220px">
            <div style="font-size:11px;color:#374151">${p.tgl_lunas ? new Date(p.tgl_lunas).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }) : ''}</div>
            <div style="position:relative;display:inline-block;margin:4px 0">
              ${companyInfo?.logo_url ? `<img src="${companyInfo.logo_url}" style="height:85px;object-fit:contain;opacity:0.18;position:absolute;left:20%;top:50%;transform:translate(-50%,-50%);z-index:0" />` : ''}
              ${companyInfo?.ttd_direktur ? `<img src="${companyInfo.ttd_direktur}" style="height:68px;object-fit:contain;position:relative;z-index:1" />` : '<div style="height:68px"></div>'}
            </div>
            <div style="font-size:11px;font-weight:600;border-top:1px solid #374151;padding-top:4px">${companyInfo?.direktur || p.lunas_nama || ''}</div>
            <div style="font-size:10px;color:#6b7280">Direktur</div>
          </div>
        </div>
        ` : ''}
      </div>`
