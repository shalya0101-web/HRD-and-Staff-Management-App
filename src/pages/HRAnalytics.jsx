...
export default function HRAnalytics() {
  ...
  async function fetchAll() {
    ...
    if (tempEmpIds.length > 0) {
      const { data: tempEmps } = await supabase.from('employees')
        ...
        .in('id', tempEmpIds)
      finalEmpList = [...empList, ...(tempEmps || []).map(e => ({ ...e, is_temporary_staff: true }))]
  ...
  function getPiketByEmployee() {
    return employees.map(emp => {
      // Total di outlet yang dipilih (termasuk penugasan sementara masuk ke outlet ini)
      const totalOutlet = piketData.filter(p => p.employee_id === emp.id).length
      // Penugasan sementara di outlet ini (lintas area masuk ke sini)
      const tugasSementaraMasuk = allPiketData.filter(p =>
        p.employee_id === emp.id && p.is_temporary
      )
      // Total semua jadwal staff ini di outlet yang dipilih
      const totalSemua = allPiketData.filter(p => p.employee_id === emp.id).length
      const outletAsal = emp.is_temporary_staff
        ? tugasSementaraMasuk[0]?.outlets?.nama || '—'
        : null
      ...
      return {
        ...
        outletSementara: outletAsal ? [outletAsal] : [],
        isTemporaryStaff: emp.is_temporary_staff || false,
