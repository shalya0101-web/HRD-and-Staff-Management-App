  ...
  return (
    <div className="space-y-4">
      ...
      <button
        ...
        className="w-full border border-blue-300 text-blue-600 hover:bg-blue-50 py-2.5 rounded-xl text-sm font-medium"
      >
...
export default function ESS() {
  ...
  async function restoreSession(session) {
    ...
    const { data } = await supabase.from('employees')
      .select('id, nama, jabatan, departemen, nik, email, no_hp, status, tgl_masuk, gaji_pokok')
      .eq('nik', nik).single()
  ...
  async function fetchAllData() {
    ...
    const [schRes, attRes, payRes, perfRes, pointRes, loanRes, leaveRes, outletRes] = await Promise.all([
  ...
  function cetakSlip(p) {
    ...
    <div class="body">
      ...
      <table>
        ...
        ${p.potongan_bpjs > 0 ? `<tr><td>BPJS</td><td style="text-align:right;color:#dc2626">- Rp ${FMT(p.potongan_bpjs)}</td></tr>` : ''}
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        {tab === 'gaji' && (
          <div className="space-y-4">
            {[gaji, jasa].filter(Boolean).map(p => (
