...
export default function Scheduling() {
  ...
  useEffect(() => { if (activeOutlet) { fetchAreaEmployees(); fetchSchedules(); fetchRequests() } }, [bulan, tahun, activeOutlet])

  async function fetchAllEmployees() {
    const { data } = await supabase
      .from('employees')
      .select('id, nama, jabatan, departemen, piket_per_bulan, status')
  ...
  function openModal(day) {
    ...
    setSlotForm(init); setError(''); setShowModal(true)
  }
  ...
  async function simpanPenugasanSementara() {
    ...
    const { error: insertErr } = await supabase.from('schedules').insert({
      ...
      outlet_id: activeOutlet,
      is_temporary: true,
      catatan_penugasan: tempForm.catatan_penugasan || 'Penugasan sementara lintas area',
      outlet_asal_id: empOutletData?.outlet_id || null,
    })

    if (insertErr) { setError('Gagal: ' + insertErr.message); setSavingTemp(false); return }

    setSuccess(`Penugasan sementara berhasil ditambahkan!`)
              ...
              {ROLE_SLOTS.map(role => {
                ...
                return (
                  <div key={role.key}>
                    ...
                    <select value={slotForm[role.key]||''} onChange={e=>setSlotForm({...slotForm,[role.key]:e.target.value})}
          ...
          <div className="bg-white rounded-xl w-full max-w-md shadow-xl">
            ...
            <div className="px-5 py-4 space-y-4">
              ...
              <div className="bg-orange-50 border border-orange-200 rounded-lg px-4 py-3 text-xs text-orange-700">
                🔀 Staff yang dipilih akan dijadwalkan di <strong>{activeOutletData?.nama}</strong> dan terekapitulasi di total piket mereka.
              </div>
              ...
              <div>
                ...
                <select value={tempForm.employee_id} onChange={e => setTempForm({...tempForm, employee_id: e.target.value})}
                  ...
                  {allEmployees
                    .filter(e => !areaEmpIds.includes(e.id)) // hanya staff dari area LAIN
                    .map(emp => {
